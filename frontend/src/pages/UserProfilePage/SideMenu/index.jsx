  ...
  return (
    <div className="py-5 bg-white border border-gray-200 rounded-lg shadow __montserat-text">
      <div
        ...
        onClick={() =>
          navigate(`${userStatus === 1 ? '/user-profile/bio' : '/user-profile/bio_seller'}`, { replace: true })
        }
      >
        <img
        ...
        <div>
          {userStatus === 1 ? (
            ...
            <div>
              <div className="ml-2 text-sm font-semibold">{staticConst.profile_menu.seller.name}</div>
              <div className="ml-2 text-xs text-gray-500">{staticConst.profile_menu.seller.role_name}</div>
            </div>
          )}
        </div>
      </div>
