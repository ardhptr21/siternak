...
const Dropdown = ({ noBorder, isLogin, loginData, dropdownOpts, title, viewMore }) => {
              ...
              >
                {isLogin && (
                  <div className="px-4 py-3">
                    ...
                    <p className="text-sm font-medium leading-5 text-gray-900 truncate">{loginData?.name || 'User'}</p>
                  </div>
                )}

                <div className="py-1">
                  {dropdownOpts &&
                    dropdownOpts.map((el, index) => (
                      <Menu.Item key={index}>
                        {({ active }) => (
                          <div
                            className={`${
                          ...
                          >
                            {el.icon && <el.icon className="mr-4 text-xl text-gray-600" />}
                            <p className={`flex justify-between w-full text-sm leading-5 text-left`}>{el?.name}</p>
                          </div>
                        )}
                      </Menu.Item>
                    ))}
                    ...
                    <Menu.Item>
                      {({ active }) => (
                        ...
                        >
                          ...
                          <BsArrowRightShort className="mr-4 text-xl text-gray-400" />
                        </div>
                      )}
                    </Menu.Item>
                  </div>
